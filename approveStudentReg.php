...
if(isset($_POST['submit']) && $_POST['submit']=='Update'){
	...
	for($i=0;$i<$tot;$i++){
		...
		$studentId	=	$_POST['student'][$i];
		$sql =	"update student_registration set status=$status where id=$studentId";
		//echo $sql;
		$db->setQuery($sql);
	}
  	...
  	<div class="columnsContainer">
	  	...
	  	<div class="leftColumn">
			...
			<table cellpadding="8">
				...
				<tr>
					<td>Student Name</td><td>Email</td><td>Username</td><td>Password</td><td>Status</td><td>Select Status</td>
				</tr>
				<?php 
				...
				<tr>
					...
					<td><?php echo $res[$i]['uname']; ?></td>
					<td><?php echo $res[$i]['pass']; ?></td>
					<td><?php if($res[$i]['status']==1)echo 'Approved'; else echo 'Pending'; ?></td>
					<td><input type="checkbox" name="status[]" id="status[]" <?php if($res[$i]['status']==1) echo "checked"; ?>> <input type="hidden" name="student[]" id="student[]" value="<?php echo $res[$i]['id']; ?>"></td>
				</tr>
				<?php 
					}
				?>
